...
def casa6_install(whl_path, select='core', user=True, upgrade=True):
    """
    install casa6 packages with pip
    """

    package_list = ['casadata']   # required?
    package_list += [whl_path]    # used by ism3d
    package_list += ['casatasks']  # used by ism3d
    if select == 'full':  # optional
        package_list += ['casashell', 'casaviewer',
